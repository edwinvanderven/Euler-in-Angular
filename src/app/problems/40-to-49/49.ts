...
export class P49 implements IProblem {
  ...
  public description = 'The arithmetic sequence, 1487, 4817, 8147, in which each of the terms increases by 3330, ' +
    ...
    for (let base = 1000; base < MAX_LENGTH; base++) {
      if (Helper.isPrime(base)) {
        for (let j = 1; j < MAX_LENGTH; j++) {
          ...
          const arePermutationsCorrect = this.isPermutation(firstPermutation, base) && this.isPermutation(secondPermutation, base);
