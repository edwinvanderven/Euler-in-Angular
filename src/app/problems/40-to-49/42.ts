...
export class P42 implements IProblem {
  public title = 'Problem 42';
  public description = 'The nth term of the sequence of triangle numbers is given by, tn = ½n(n+1); so the first ten triangle numbers are:' +
                        '\n1, 3, 6, 10, 15, 21, 28, 36, 45, 55, ...' +
                        '\nBy converting each letter in a word to a number corresponding to its alphabetical position and adding these values we form a word value.' +
                        '\nFor example, the word value for SKY is 19 + 11 + 25 = 55 = t10. If the word value is a triangle number then we shall call the word a triangle word.' +
                        '\nUsing words.txt (right click and \'Save Link/Target As...\'), a 16K text file containing nearly two-thousand common English words, how many are triangle words?';

  private triangleNumbers = [1, 3, 6, 10, 15, 21, 28, 36, 45, 55, 66, 78, 91, 105, 120, 136, 153, 171, 190];
  ...
  getAnswer(): string {
    ...
    this.words.forEach((word: string) => {
      if (this.isTriangleNumber(this.getWordValue(word))) result++;
    });
    return result.toString();
  }

  private isTriangleNumber(x: number): boolean {
    // precalculated to increase performance
    if (this.triangleNumbers.indexOf(x) > -1) return true;

    // const length = 20;
    // for (let i = 1; i < length; i++) {
    //   const triangleNumber = Math.floor(i * (i + 1) / 2);
  ...
  private getWordValue(word: string): number {
    let sum = 0;
    const length = word.length;
    for (let i = 0; i < length; i++) {
      sum += word.charCodeAt(i) - 65  + 1;
    }
    return sum;
  }
}
